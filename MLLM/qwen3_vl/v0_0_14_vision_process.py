...
VIDEO_READER_BACKENDS = {
    "decord": _read_video_decord_frame_idx,
...
def fetch_video_frame_idx(ele: Dict[str, Any], image_patch_size: int = 14, return_video_sample_fps: bool = False,
                          return_video_metadata: bool = False, frame_idx: List[int] = None, nframes: int = None) -> Union[torch.Tensor, List[Image.Image]]:
    ...
    if isinstance(ele["video"], str):
        ...
        try:
            video, video_metadata, sample_fps = VIDEO_READER_BACKENDS[video_reader_backend](ele, frame_idx=frame_idx, nframes=nframes)
...
def process_vision_info_frame_idx(
    ...
    for vision_info in vision_infos:
        if "image" in vision_info or "image_url" in vision_info:
            ...
            video_input, video_sample_fps = fetch_video_frame_idx(vision_info, return_video_sample_fps=True,
                        image_patch_size=image_patch_size, return_video_metadata=return_video_metadata, frame_idx=frame_idx, nframes=nframes)
